...
class DistillationActorRolloutRefWorker(ActorRolloutRefWorker):
    """
    actor 和 rollout 模型一致，但是 reference 模型是 distillation 时的 teacher model
    """
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self):
        from verl.workers.actor import DataParallelPPOActor
        ...
        if self._is_ref:
            local_path = copy_to_local(self.config.ref.model.path, use_shm=use_shm)
            override_model_config = OmegaConf.to_container(self.config.ref.model.get("override_config", OmegaConf.create()))  # noqa: E501

            use_remove_padding = self.config.ref.model.get("use_remove_padding", False)
            use_shm = self.config.ref.model.get("use_shm", False)
            use_fused_kernels = self.config.ref.model.get("use_fused_kernels", False)
            self.ref_module_fsdp = self._build_model_optimizer(
                model_path=local_path,
                fsdp_config=self.config.ref.fsdp_config,
                optim_config=None,
                ...
                use_remove_padding=use_remove_padding,
                use_fused_kernels=use_fused_kernels,
                trust_remote_code=self.config.ref.model.get("trust_remote_code", False),
                use_liger=self.config.ref.model.get("use_liger", False),
                role="ref",
            )[0]
            OmegaConf.set_struct(self.config.ref, True)
            with open_dict(self.config.ref):
