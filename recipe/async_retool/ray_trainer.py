...
from verl.trainer.ppo.ray_trainer import compute_response_mask, compute_advantage, apply_kl_penalty, agg_loss
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.filter_groups.enable:
                        ...
                        print(f"[DEBUG][Trainer] Valid prompt: {len(kept_prompt_uids)=}")

                        # repeat batch to align with self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n
                        if len(batch) < self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n:
                            repeat_times = self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n // len(batch) + 1
                            batch = batch.repeat(repeat_times=repeat_times, interleave=False)
                            batch = batch[:self.config.data.train_batch_size * self.config.actor_rollout_ref.rollout.n]
