...
- Due to potential incomplete memory offloading during sleep operations for vLLM on NPUs, we added patches to manually handle the offloading and onloading of the rollout model on NPUs. The related code is in `megatron_vllm.py` and `megatron_workers.py`.
