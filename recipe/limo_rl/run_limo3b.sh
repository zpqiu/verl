...
# 检查 HOME_DIR 环境变量是否存在
if [ -z "$HOME_DIR" ]; then
    echo "错误: HOME_DIR 环境变量未设置"
    echo "请设置 HOME_DIR 环境变量后再运行此脚本"
    exit 1
fi

# 检查 HOME_DIR 目录是否存在
if [ ! -d "$HOME_DIR" ]; then
    echo "错误: HOME_DIR 目录不存在: $HOME_DIR"
    exit 1
fi

current_dir="$(dirname "$(readlink -f "$0")")"

project_name='alexq_qwen2_5_limo_rl'
exp_name='R1-Skywork-OR1-3B'
...
TRAIN_FILE=${TRAIN_FILE:-"${HOME_DIR}/data/limo_rl/skywork_or1/train.parquet"}
...
ray job submit --runtime-env="${RUNTIME_ENV}" \
    ...
    data.val_files="[$HOME_DIR/data/limo_rl/aime24/test.parquet,$HOME_DIR/data/limo_rl/math500/test.parquet]" \
